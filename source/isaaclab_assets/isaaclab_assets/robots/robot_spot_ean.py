...
QUAD_EAN = ArticulationCfg(
    spawn=sim_utils.UsdFileCfg(
        ...
        #usd_path="/home/manh/Projects/IsaacLab/spot_micro_rviz/urdf/spot_micro.usd",
        ...
        articulation_props=sim_utils.ArticulationRootPropertiesCfg(
            ...
            solver_position_iteration_count=4,  # Tăng để cải thiện độ chính xác va chạm
            solver_velocity_iteration_count=0,   # Thêm lặp vận tốc để mô phỏng chuyển động mượt hơn
        ),
        scale=(2., 2., 2.)
    ),
    
    init_state=ArticulationCfg.InitialStateCfg(
        pos=(0.0, 0.0, 0.6),  # Giảm z xuống 0.25m để chân dễ chạm đất hơn
        joint_pos={
            ".*_left_shoulder": 0.0,    # Vai trung lập
            ".*_right_shoulder": 0.0,   # Vai trung lập
            ".*_leg": -0.5,             # Chân gập nhẹ (-0.5 rad ~ -28 độ), tư thế đứng tự nhiên hơn
            ".*_foot": 1.0,             # Bàn chân cong xuống (1 rad ~ 57 độ), đủ để chạm đất
        },
        joint_vel={".*": 0.0},  # Vận tốc ban đầu vẫn là 0
    ),
    soft_joint_pos_limit_factor=100.0,  # Tăng lên 95% để tận dụng phạm vi khớp thực tế hơn
    actuators={
        "spot_hip": DelayedPDActuatorCfg(
            joint_names_expr=hxy,
            effort_limit=45.0,
            stiffness=60.0,
            damping=1.5,
